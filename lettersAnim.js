...
const animate = () => {
    ...
    if(window.foundWords && Object.keys(window.foundWords).length) {
        ctx.fillStyle = "white";
        ...
        for(let i = 0; i < Math.min(Object.keys(window.foundWords).length, 6); i += 1) {
            const word = Object.keys(window.foundWords)[i];

            ctx.fillText(word + " - " + word.length, 1150, 100 + i * 60);
        }
    }
...
const drawBg = function() {
    ...
    ctx.font = "bold 48px arial";
    ctx.textAlign = "left";
    ctx.fillText("VOWEL", 10, 100);
    ctx.fillText("CONSONANT", 10, 180);
